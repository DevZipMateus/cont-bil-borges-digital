    ...
    >
      <div className="container-custom flex items-center justify-between">
        <a href="#inicio" className="flex items-center gap-3">
          <img src={logo} alt="Hagios Borges Contabilidade" className="h-12 w-auto" />
        </a>
      ...
      {/* Mobile Navigation */}
      {isMobileMenuOpen && (
        <nav className="md:hidden bg-card/98 backdrop-blur-md border-t border-border mt-3 py-4">
          <div className="container-custom flex flex-col gap-4">
            {navLinks.map((link) => (
              <a
                ...
                className="text-foreground hover:text-primary transition-colors duration-200 py-2 text-base font-medium"
              >
                {link.label}
              </a>
            ))}
